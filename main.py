...
groups = list(map(lambda x: Group.create_or_get(**x)[0], groups_parser(requests.get(baseUrl + 'students').content)))
...
for group in groups:
    ...
    students = list(map(lambda x: Student.create_or_get(**x)[0].groups.add(group),
                        student_list_parser(requests.post(baseUrl + 'students', data=data).content)))
...
for student in students:
    for disciplineDict in student_parser(requests.get(baseUrl + 'marks_student?id={}'.format(student.id)).content):
